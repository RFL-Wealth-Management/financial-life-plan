...
// "$57,816/year" from 57816. Null -> "". Matches the bucket "What It Delivers"
// cells (mirrors the IFLP's per-year formatting).
function perYear(n: number | null): string {
  const s = formatCurrency(n);
  return s ? `${s}/year` : "";
}

// Sum that stays null until at least one input is present, so an empty section's
// total renders blank rather than "$0".
function sumOrNull(values: (number | null)[]): number | null {
  const present = values.filter((v): v is number => v != null);
  return present.length ? present.reduce((a, b) => a + b, 0) : null;
}

const times12 = (n: number | null): number | null => (n == null ? null : n * 12);
...
export function buildFflpDocPayload(
  ...
  const cpp1A = times12(s.govCpp1Monthly);
  const cpp2A = times12(s.govCpp2Monthly);
  const oas1A = times12(s.govOas1Monthly);
  const oas2A = times12(s.govOas2Monthly);
  ...
  return {
    ...
    bucketGovAnnual: perYear(govTotalAnnual),
    bucketPensionMonthly: c(pensionTotalMonthly),
    bucketPensionAnnual: perYear(pensionTotalAnnual),
    bucketCorpMonthly: c(s.corpMonthly),
    bucketCorpAnnual: perYear(s.corpAnnual),
    bucketInsuranceMonthly: c(insMonthlyTotal),
    bucketInsuranceAnnual: perYear(insAnnualTotal),
